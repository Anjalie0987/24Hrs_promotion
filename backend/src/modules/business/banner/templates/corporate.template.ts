...
try {
  ...
  if (fs.existsSync(regularFontPath)) {
    GlobalFonts.registerFromPath(regularFontPath, 'Roboto');
  }
...
// 1080x1080 Square Layout for Social Media
const layout = {
  width: 1080,
  height: 1080,
  margin: 60,
...
export async function generateCorporateBanner(
  data: BannerData,
): Promise<Buffer> {
...
/**
 * 1. Background (Glassmorphism & Shapes)
 */
function drawBackground(ctx: SKRSContext2D) {
  ...
  // Subtle Premium Geometric Shapes
...
/**
 * 2. Header (Verified Badge + Platform Logo)
 */
async function drawHeader(ctx: SKRSContext2D, startY: number): Promise<number> {
  const topY = startY;
  ...
  ctx.fill(new Path2D('M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z'));
  ctx.restore();
  ...
  try {
    const logoPath = path.join(
      process.cwd(),
      '../frontend/public/24HR_logo.png',
    );
    const platformLogo = await loadImage(logoPath);
    ...
    ctx.fillText('24Hrs Promotion', rightX, topY + 35);
...
/**
 * 3. Middle Section: Business Hero, Description, Services
 */
async function drawBusinessSection(
  ctx: SKRSContext2D,
  data: BannerData,
  startY: number,
): Promise<number> {
...
/**
 * 4. Lower Section: Owner Card & Quick Contact
 */
async function drawOwnerCard(
  ctx: SKRSContext2D,
  data: BannerData,
  startY: number,
): Promise<number> {
  ...
  if (data.ownerPhotoUrl) {
    try {
      const owner = await loadImage(data.ownerPhotoUrl);
      ctx.save();
  ...
  const contactW = availableW / 3;

  drawContactItem(
    ctx,
    'phone',
    data.whatsapp || '+1 234 567 890',
    textX,
    textY,
    contactW,
  );
 ...
 * 5. Footer Watermark
 */
function drawFooter(ctx: SKRSContext2D) {
  const footerY = layout.height - 35;
  ctx.fillStyle = colors.muted;
  ctx.font = '18px Roboto';

  const textLeft = 'Generated by 24Hrs Promotion';
  const textRight = 'www.24hrspromotion.com';
