...
async function drawOwnerCard(
  ...
  if (data.whatsapp) items.push({ type: 'phone', text: data.whatsapp });
  if (data.email) items.push({ type: 'mail', text: data.email });
  if (data.website)
    items.push({
      type: 'globe',
      text: data.website.replace(/^https?:\/\/(www\.)?/, ''),
    });
  if (data.location) items.push({ type: 'location', text: data.location });
